...
export default function Home() {
  return <main id="top"><Header />
    ...
    <section className="intro section-light" id="officina">
    ...
    <section className="services-preview section-dark" id="servizi">
      <div className="section-heading"><div><p className="eyebrow">I SERVIZI PRINCIPALI</p><h2>TUTTO QUELLO CHE SERVE.<br /><span>IN UN’UNICA OFFICINA.</span></h2></div></div>
      <div className="service-preview-grid">{featured.map(([title,text],i)=><Link href="#contatti" key={title}><small>0{i+1}</small><h3>{title}</h3><p>{text}</p><span>CHIEDI INFORMAZIONI →</span></Link>)}</div>
    </section>

    <section className="roadside" id="soccorso"><div><p className="eyebrow red">QUANDO L’AUTO SI FERMA</p><h2>NON RESTARE FERMO.<br /><span>CI PENSIAMO NOI.</span></h2></div><div><p>Soccorso stradale, recupero del veicolo e trasporto in officina. Cristian è il riferimento per assistenza e appuntamenti.</p><Link className="text-link" href="#contatti">RICHIEDI ASSISTENZA →</Link></div></section>
    <div id="contatti"><FinalCta eyebrow="AUTO SERVICE UNO · MILANO" title="PRENOTA IL TUO PROSSIMO INTERVENTO." text="Raccontaci cosa serve alla tua auto: organizziamo insieme controllo, tempi e lavorazione." /></div>
    <Footer />
  </main>;
}
