...
export default function Servizi(){return <main><Header /><section className="page-head"><p className="eyebrow">SERVIZI COMPLETI</p><h1>CI PRENDIAMO CURA<br /><span>DELLA TUA AUTO.</span></h1><p>Dalla diagnosi al soccorso stradale: spiegazioni chiare, lavori concordati e un team sempre raggiungibile.</p></section><section className="detailed-services">{services.map(s=><article key={s.n}><h2>{s.title}</h2><p>{s.intro}</p><ul>{s.items.map(item=><li key={item}>{item}</li>)}</ul><Link href="/contatti">CHIEDI INFORMAZIONI →</Link></article>)}</section><section className="friendly-cta"><div><p className="eyebrow">NON SAI QUALE SERVIZIO SERVE?</p><h2>RACCONTACI IL PROBLEMA.</h2><p>Ti aiutiamo a capire il controllo giusto senza complicazioni.</p></div><Link className="button red-button" href="/contatti">PARLA CON NOI →</Link></section><Footer /></main>}
